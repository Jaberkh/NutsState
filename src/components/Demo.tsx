...
const fetchUsernames = async (fids: number[]): Promise<Record<number, string>> => {
  ...
  const options = {
    ...
    headers: {
      ...
      "x-api-key": "6DE29E6C-74A6-466A-A41E-AA2C362CC97A",
    },
  };
  try {
                ...
                srcDoc={`<html><head><style>
                  ...
                  tippedTodayCasts.length > 0
                    ? tippedTodayCasts
                        .map(
                          (cast) =>
                            `<div class="cast"><div class="author">Recipient: @${cast.author.username} (FID: ${cast.author.fid})</div>${cast.text}</div>`
                        )
                        .join("")
